...
Dual-write during the transition: the per-half file is what current readers prefer, the
legacy aggregate keeps consumers pinned to an older cotdata working.
"""
import json
...
# ── dual write ────────────────────────────────────────────────────────────
def test_write_lands_in_both_the_half_and_the_legacy_aggregate(store_env):
    from cotdata import config, store
    store.write_prices("ES", "backadj", _prices(), source="test")

    half = json.loads(config.manifest_path_for("prices").read_text())
    assert half["prices"]["ES_backadj"]["n_rows"] == 3
    legacy = json.loads(config.manifest_path().read_text())
    assert legacy["prices"]["ES_backadj"]["n_rows"] == 3
...
def test_a_lost_legacy_write_does_not_lose_the_entry(store_env):
    """The hazard, simulated: the shared aggregate is clobbered by a concurrent
    producer. Readers must still see both entries, because they prefer the halves."""
    from cotdata import config, store
    store.write_prices("ES", "backadj", _prices(), source="test")
    store.write_cot_legacy("ES_13874A", _cot(), source="test")
