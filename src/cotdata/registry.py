"""The single symbol registry: internal symbol ↔ CFTC code ↔ asset class.
Replaces the scattered maps (CotSymbolCodeMap and friends).
...
CFTC POSITIONING ONLY, since ADR-0007. The price-vendor mappings this file used to
carry (norgate, yahoo, databento) live in marketdata's registry now, beside the
producers that read them. The two registries are separate files and cannot be
merged: this loader requires a `cftc_code` of every symbol, and equities have no
COT report — which is the same requirement that made a separate package necessary
in the first place.
...
class Symbol:
    ...
    # NO VENDOR MAPPINGS, and none should come back. The norgate / yahoo / databento
    # columns, the `price_source` override and the whole resolve_source machinery moved
    # to marketdata's registry with the producers that read them (ADR-0007).
    #
    # An earlier note here claimed they had to stay because a deployment might SHARE one
    # registry file between the two packages via COTDATA_REGISTRY. That was wrong: this
    # loader hard-requires `cftc_code` and marketdata's equities do not have one, so the
    # two files can never be the same file.
